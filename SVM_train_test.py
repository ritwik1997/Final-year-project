...
#X_MDS = manifold.MDS(n_components=800).fit_transform(X)
X_LLE = manifold.LocallyLinearEmbedding(n_components=500).fit_transform(X)
X_train, X_test, y_train, y_test = model_selection.train_test_split(X_LLE, y, test_size=0.2)
...
#X_SpectralEmbedding = manifold.SpectralEmbedding(n_components=500).fit_transform(X)
...
clf.fit(X_SpectralEmbedding,y)
y1=[]
correct = 0
for i in range(len(X_SpectralEmbedding)):

    predict_me = np.array(X_SpectralEmbedding[i].astype(float))
...
print(correct/len(X_SpectralEmbedding)*100)
